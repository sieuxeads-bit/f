...
class KokoroSrtApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.status_var = tk.StringVar(
            value="Chọn model ONNX + voices + SRT để bắt đầu."
        )
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            root, text="Kokoro SRT Local", font=("Segoe UI", 20, "bold")
        ).pack(anchor="w")
        ttk.Label(
            root,
            text=(
                "Đọc giọng tự nhiên: không ép câu theo ô SRT. "
                "Nếu câu trước chưa xong, câu sau sẽ chờ để tránh chồng giọng."
            ),
        ...
        self.voice_combo = ttk.Combobox(
            settings, textvariable=self.voice_var, state="readonly"
        )
        self.voice_combo.grid(row=0, column=1, sticky="ew", pady=5)
        ttk.Button(settings, text="Nạp voices", command=self.load_model).grid(
            row=0, column=2, padx=8, pady=5
        )

        ttk.Label(settings, text="Language").grid(
            row=0, column=3, sticky="e", padx=(8, 6), pady=5
        )
        ttk.Entry(settings, textvariable=self.lang_var, width=12).grid(
        ...
        speed_frame.grid(row=1, column=1, columnspan=4, sticky="ew", pady=5)
        ...
        self.speed_var.trace_add(
            "write",
            lambda *_: self.speed_label.configure(
        ...
        self.run_btn = ttk.Button(
            actions, text="Tạo voice từ SRT", command=self.start_generate
        )
        self.run_btn.pack(side="left")
        ttk.Button(
            actions, text="Mở thư mục output", command=self.open_output_folder
        ...
        ttk.Label(
            root, textvariable=self.status_var, wraplength=850
    ...
    def _path_row(self, parent, label: str, var: tk.StringVar, command) -> None:
        ...
        ttk.Button(row, text="Chọn...", command=command, width=10).pack(
            side="right"
        )
    ...
    def _load_model_worker(self, model: Path, voices: Path) -> None:
        try:
            ...
            self._events.put(
                ("model_loaded", (kokoro, available, str(model), str(voices)))
            )
        except Exception as exc:
    ...
    def _generate_worker(
        ...
        try:
            ...
            for i, cue in enumerate(cues, start=1):
                self._events.put(
                    ("status", f"[{i}/{len(cues)}] {cue.text[:80]}")
                )
                ...
                if sample_rate is None:
                    sample_rate = sr
                    initial_seconds = max(cue.end for cue in cues) + 1.0
                    timeline = np.zeros(
                        int(np.ceil(initial_seconds * sample_rate)),
                        dtype=np.float32,
                    )
                elif sr != sample_rate:
                ...
                if start_sample > requested_start:
                    delay = (start_sample - requested_start) / sample_rate
                    self._events.put(
                        (
                            "log",
                            f"Cue {cue.index} chờ {delay:.2f}s để câu trước đọc xong.",
                        )
                    )
            ...
            assert timeline is not None and sample_rate is not None
            final_length = max(cursor_sample, 1)
            timeline = timeline[:final_length]
